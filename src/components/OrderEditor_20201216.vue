...
<v-container fluid>
    ...
    <v-card height="80vh">
            ...
            <v-container class="ma-0 pb-0" fluid>
                <v-row class="pb-0">
                    ...
                    <v-col cols="2" class="pb-0">
                        <v-text-field dense v-model="data.name" label="Name" :rules="rules" @change="isModified = true"></v-text-field>
                    </v-col>
                </v-row>
            </v-container>
        </v-form>
        ...
        <v-container class="ma-0" fluid>
            <v-row no-gutters>
                <v-col cols="5">
                    ...
                    <v-btn :color="isModified ? 'primary' : ''" :disabled="!isModified" class="ma-2" fab @click="save">
                        <v-icon>mdi-content-save</v-icon>
                    </v-btn>
                    <v-btn class="ma-2" fab :disabled="!parts.items.length" @click="deletePartsFromOrder()">
                        <v-icon dark>mdi-trash-can-outline</v-icon>
                    </v-btn>

                    <v-btn color="primary" class="ma-2" fab :disabled="parts.items.length>0" :loading="isSelecting" @click="onFileUpload">
                        <v-icon dark>mdi-cloud-upload</v-icon>
                    </v-btn>

                    <v-btn class="ma-2" fab @click="showFilteringDialog" :disabled="!parts.items.length">
                        <v-icon dark>mdi-filter-outline</v-icon>
                    </v-btn>
                    <v-btn class="ma-2" fab @click="undoPartsSelection" :disabled="!selectedParts.length">
                        <v-icon dark>mdi-filter-off-outline</v-icon>
                    </v-btn>

                    <v-btn color="warning" class="ma-2" fab @click="distributeParts" :disabled="!selectedParts.length || !selectedCarts.length">
                        <v-icon dark>mdi-card-plus</v-icon>
                    </v-btn>

                    <v-btn color="danger" class="ma-2" fab @click="undoPartsDistribution" :disabled="!selectedParts.length">
                        <v-icon dark>mdi-card-remove-outline</v-icon>
                    </v-btn>

                    <v-badge light top overlap :content="logsData.length" :value="logsData.length">
                ...
                <v-col cols="4" v-if="selectedParts.length">
                    <v-select v-model="selectedCarts" :items="availableCarts" chips :deletable-chips="true" label="Carts for distribution" multiple outlined></v-select>
                </v-col>
        ...
        <v-data-table fixed-header dense height="65vh" ref="dataTable" v-model="selectedParts" :headers="parts.headers" :items="parts.items.filter(part => !part.deleted)" :search="search" :items-per-page="50" :footer-props="{'items-per-page-options': [10, 25, 50, 100, 200]}" show-select :single-select="false" item-key="_uid" class="elevation-1">
            <template v-slot:header="{ props: { headers } }">
                <tr>
                    <td class="pl-1" v-for="(h,index) in headers.filter( h=>h.value!=='action')" :key="`column-filter-${index}`">
                        <v-select v-if="index === 0" :items="[{text: 'All', value: 0},{text: 'Yes', value: 1},{text: 'No', value: 2}]" v-model="columnFilters.selected" label=""></v-select>
                        <v-text-field v-if="index!==0" v-model="columnFilters[h.value]" type="text" label="" clear-icon="mdi-close-circle" clearable></v-text-field>
                    </td>
                </tr>
            </template>
            <template v-slot:item="{ item, isSelected, select }">
                <tr :class="isSelected?'yellow':''" v-if="(columnFilters.selected === 1 && isSelected) || (columnFilters.selected === 2 && !isSelected) || (!columnFilters.selected)">
                    <!-- class="d-flex align-center" -->
                    <td @click="toggle(isSelected,select,$event)">
                        <v-icon class="px-1" v-if="isSelected">mdi-checkbox-marked</v-icon>
                        <v-icon class="px-1" v-if="!isSelected">mdi-checkbox-blank-outline</v-icon>
                    </td>
                    <td @click="toggle(isSelected,select,$event)">{{item["ORDERNAME"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["ART ID"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["ID"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["TYP"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["NAME1"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["MPE TYPE"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["PARENT ID"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["ART POSSTR"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["PARTPOSSTR"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["NCNO"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["_width_decimal"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["_length_decimal"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["_thickness_decimal"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["MATNAME"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["BARCODE"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["_cart_alias"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">{{item["_bin_order"]}}</td>
                    <td @click="toggle(isSelected,select,$event)">
                        <v-progress-circular indeterminate color="primary" :size="25" v-if="item._loading"></v-progress-circular>

                        <span v-if="!item._loading">{{item["_bin_state"]}}</span>
                    </td>
                    <td @click="toggle(isSelected,select,$event)">{{getPartStateAlias(item["_state"])}}</td>

                    <td>
                        <v-btn-toggle>
                            <v-btn text icon @click="selectBinForPart(item)" :disabled="selectedParts && selectedCarts.length === 0 || (item.hasOwnProperty('_bin_uid') && item._bin_uid !== null)">
                                <v-icon> mdi-card-plus </v-icon>
                            </v-btn>

                            <v-btn text icon @click="findBinForPart(item)" :disabled="selectedParts && selectedCarts.length === 0 || (item.hasOwnProperty('_bin_uid') && item._bin_uid !== null)">
                                <v-icon> mdi-card-search-outline </v-icon>

                            </v-btn>
                            <v-btn text icon @click="removePartFromBin(item)" :disabled="!item._bin_uid">
...
<v-row class="pa-0">
            <v-chip class="ml-3" label dark v-if="selectedParts.length>0">Selected {{selectedParts.length}} parts </v-chip>
            <v-chip class="ml-3" label dark v-if="selectedFilter"> Selected filter: {{selectedFilter.name}} [{{selectedFilter.description}}]</v-chip>
        </v-row>
            </template>
...
-->
    ...
    <filtering-dialog ref="filtering-dialog" @save="onFilteringDialogApply" @close="onFilteringDialogClose"> </filtering-dialog>
    <logs-dialog ref="logs-dialog" @clear="onLogsDialogClear"> </logs-dialog>
    <distribution-dialog ref="distribution-dialog" :carts="availableCarts" @run="onDistributionDialogRun" @clear="onDistributionDialogClear"> </distribution-dialog>
    <bin-selector-dialog ref="bin-selector-dialog" @save="onBinSelectorDialogApply" @close="onBinSelectorDialogClose"> </bin-selector-dialog>
...
export default {
    ...
    computed: {
        ...
        activeCarts() {
            ...
            return this.fullDataSet.filter(c => c.status === this.$constants.CART_STATUS.ACTIVE).map(c => ({
                text: c.alias,
                value: c.id
            }))
    ...
    async mounted() {
        ...
        if (!this.id) {
            ...
            {
                let [error, response] = await to(that.getOrder({
                    id: this.id,
                }));
...

            {
                ...
                let [error, response] = await to(this.getOrderParts({
                    _order_uid: this.id,
                }));
    ...
    methods: {
        ...
        filterByColumn(value, column) {
            ...
            let search = this.columnFilters[column] ? this.columnFilters[column].toString().toLowerCase() : null
...
if (!this.parts.items){
    debugger
            ...
            this.parts.items.forEach(part => {
                ...
                part._bin_state = bin ? `${bin.parts.length} / ${bin.maxPartsCount}` : ''
        ...
        async distributeParts() {
            ...
            if (!this.fullDataSet) {
                await this.$refs.dialog.open("Error", `Error occurred during fetching carts data`, {
                    OK: "OK"
                });
            ...
            */
            ...
            //console.object('sorted parts',sortedParts.map( p => ({ id: p["ID"], articleMaxSize: p.articleMaxSize })))
...
*/
        ...
        updateColumn(part, property, value) {
            ...
            part[property] = value
            let partIdx = this.parts.items.findIndex(item => item._uid === part._uid);
        ...
        findSuitableBin(part, selectedCarts = undefined) {
            ...
            if (!selectedCarts) {
                ...
                dataSet = this.fullDataSet.filter(cart => cart.status === this.$constants.CART_STATUS.ACTIVE && this.partTypeAllowed(cart, part) && selectedCarts.includes(cart.id))
            }
            ...
            for (let ic = 0; ic < dataSet.length; ic++) {
                ...
                let potentialBins = cart.bins.filter( b => (
                            //fit by Article AND OrderName
                            (b.parts.length > 0 && b.parts[0]["ART ID"] === part["ART ID"] && b.parts[0]["ORDERNAME"] === part["ORDERNAME"]) || 
                            //or empty
                            (b.parts.length === 0)
                        ) &&
                        //fit by Size
                ...
                for (let ib = 0; ib < potentialBins.length; ib++) {
                    ...
                    if (bin.parts.length)
                        bin.totalPartsThickness = bin.parts.map(o => o._thickness_decimal).reduce((a, c) => +a + +c)
                    else
                        bin.totalPartsThickness = 0
                    ...
                    if (bin.parts.length < bin.maxPartsCount) {
                        ...
                        //by axes
                        if (part._length_decimal < bin.width && part._width_decimal < bin.depth ) axis = 'X' 

                        //X-90
                        else if (part._length_decimal < bin.depth && part._width_decimal < bin.width ) axis = 'X-90'
                        
                        //Y axis
                        else if (part._length_decimal < bin.height && part._width_decimal < bin.depth ) axis = 'Y'

                        //Y-90 axis
                        else if (part._length_decimal < bin.depth && part._width_decimal < bin.height ) axis = 'Y-90'

                        if (axis !== undefined){
                            this.addLogRecord(part, `CART: ${cart.alias}, BIN: ${bin.order}, Axis ${axis}, AMS: ${part.articleMaxSize}, PMS: ${part.partMaxSide}`)
            ...
            return [false, false, `Bin not found, AMS: ${part.articleMaxSize}, PMS: ${part.partMaxSide}`]
        },
        ...
        findSuitableBin_old(part, selectedCarts = undefined) {
            ...
            for (let ic = 0; ic < dataSet.length; ic++) {
                ...
                for (let ib = 0; ib < cart.bins.length; ib++) {
                    ...
                    if (bin.parts.length < bin.maxPartsCount && part._width_decimal < bin.width
                        //&& (bin.totalPartsThickness + p._thickness_decimal < p.height_decimal)
                        && (bin.parts.length === 0 || (bin.parts[0]["ART ID"] === part["ART ID"] && bin.parts[0]["ORDERNAME"] === part["ORDERNAME"]))
                    ) {
        ...
        partTypeAllowed(cart, part) {
            ...
            if (cart.type === this.$constants.CART_TYPE.FACE_FRAME_PARTS && this.$constants.PART_TYPES.FACE_FRAME_PARTS.includes(+part["TYP"])) return true

            if (cart.type === this.$constants.CART_TYPE.MULTI_5_PIECE_FRONTS_PARTS && this.$constants.PART_TYPES.MULTI_5_PIECE_FRONTS_PARTS.includes(+part["TYP"])) return true

            if (cart.type === this.$constants.CART_TYPE.DRAWER_BOX_PARTS && this.$constants.PART_TYPES.DRAWER_BOX_PARTS.includes(+part["TYP"])) return true
        ...
        async undoPartsDistribution() {
            ...
            this.selectedParts.filter(p => p._bin_uid).forEach(p => {
        ...
        */
        ...
        onFileLoading(isLoading) {
            ...
            if (!isLoading) {
                ...
                this.parts.items.forEach((i) => {
                    ...
                    i._uid = this.$nanoid();
                    i._width_decimal = this.$helpers.fraction2decimal(i["Width"].replace('in', ''))
                    i._length_decimal = this.$helpers.fraction2decimal(i["Length"].replace('in', ''))
                    i._thickness_decimal = this.$helpers.fraction2decimal(i["Thickness"].replace('in', ''))
                    i._state = 0
                    i.created = true
        ...
        onFilteringDialogApply(data) {
            ...
            if (filter.id === this.$constants.FILTERS.CARCASS_PARTS) {
                ...
                this.selectedParts = this.$helpers.mergeObjectArrays(this.selectedParts, this.parts.items.filter(p => this.$constants.PART_TYPES.CARCASS_PARTS.includes(+p["TYP"])), '_uid')
            ...
            if (filter.id === this.$constants.FILTERS.DRAWER_BOX_PARTS) {
                ...
                this.selectedParts = this.$helpers.mergeObjectArrays(this.selectedParts, this.parts.items.filter(p => this.$constants.PART_TYPES.DRAWER_BOX_PARTS.includes(+p["TYP"])), '_uid')
            ...
            if (filter.id === this.$constants.FILTERS.DOOR_DRAWER_FRONTS_SINGLE) {
                ...
                this.selectedParts = this.$helpers.mergeObjectArrays(this.selectedParts, result, '_uid')
            }

            if (filter.id === this.$constants.FILTERS.FACE_FRAME_PARTS) {
                /*TYP: 36*/

                this.selectedParts = this.$helpers.mergeObjectArrays(this.selectedParts, this.parts.items.filter(p => this.$constants.PART_TYPES.FACE_FRAME_PARTS.includes(+p["TYP"])), '_uid')

            }
            if (filter.id === this.$constants.FILTERS.CARCASS_NOP_MULTI_AND_DRAWER_BOX_MULTI) {
                /*TYP: 14, 5,4,1,13,6,12,3,34, 21,22. Exclude parents */

                const result = this.$filters.getChildsWithoutParents(this.parts.items, {
                    typ: this.$constants.PART_TYPES.CARCASS_NOP_MULTI_AND_DRAWER_BOX_MULTI
                })

                this.selectedParts = this.$helpers.mergeObjectArrays(this.selectedParts, result, '_uid')

            }
            if (filter.id === this.$constants.FILTERS.CARCASS_NOP_MULTI_AND_DRAWER_BOX_SINGLE) {
                ...
                this.selectedParts = this.$helpers.mergeObjectArrays(this.selectedParts, result1, '_uid')
                this.selectedParts = this.$helpers.mergeObjectArrays(this.selectedParts, result2, '_uid')
            ...
            if (filter.id === this.$constants.FILTERS.ALL_PARTS) {
                ...
                this.selectedParts = this.$helpers.mergeObjectArrays(this.selectedParts, this.parts.items, '_uid')
            ...
            if (filter.id === this.$constants.FILTERS.MULTI_5_PIECE_FRONTS_PARTS) {
                /*
                5 Piece Door/Drawer Fronts
                All TYP: 8,9,10,23 from the same MPE i.e get parts with typ=8 and mpetype=1 + all child parts
                */
    ...
    watch: {
        'selectedParts'(newValue) {
            ...
            if (this.selectedFilter && this.selectedFilter.id === this.$constants.FILTERS.FACE_FRAME_PARTS) {
                //select carts FACE_FRAME_PARTS
                this.selectedCarts = this.fullDataSet.filter(cart => cart.status === this.$constants.CART_STATUS.ACTIVE && cart.type === this.$constants.CART_TYPE.FACE_FRAME_PARTS).map(c => c.id)

            } else if (this.selectedFilter && this.selectedFilter.id === this.$constants.FILTERS.MULTI_5_PIECE_FRONTS_PARTS) {
                //select carts MULTI_5_PIECE_FRONTS_PARTS
                this.selectedCarts = this.fullDataSet.filter(cart => cart.status === this.$constants.CART_STATUS.ACTIVE && cart.type === this.$constants.CART_TYPE.MULTI_5_PIECE_FRONTS_PARTS).map(c => c.id)

            } else if (this.selectedFilter && this.selectedFilter.id === this.$constants.FILTERS.DRAWER_BOX_PARTS) {
                //select carts DRAWER_BOX_PARTS
                this.selectedCarts = this.fullDataSet.filter(cart => cart.status === this.$constants.CART_STATUS.ACTIVE && cart.type === this.$constants.CART_TYPE.DRAWER_BOX_PARTS).map(c => c.id)
